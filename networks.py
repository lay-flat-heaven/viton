...
class FeatureExtraction(nn.Module):
    def __init__(self, input_nc, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super().__init__()
        downconv = nn.Conv2d(input_nc, ngf, kernel_size=(4,4), stride=(2,2), padding=1)
        model = [downconv, nn.ReLU(True), norm_layer(ngf)]
        for i in range(n_layers):
            in_ngf = 2**i * ngf# if 2**i * ngf < 512 else 512
            out_ngf = 2**(i+1) * ngf# if 2**i * ngf < 512 else 512
            downconv = nn.Conv2d(in_ngf, out_ngf, kernel_size=(4,4), stride=(2,2), padding=1)
            model += [downconv, nn.ReLU(True)]
            model += [norm_layer(out_ngf)]
        model += [nn.Conv2d(512, 512, kernel_size=(3,3), stride=(1,1), padding=1), nn.ReLU(True)]
...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=512,output_dim=6, use_cuda=True):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(input_nc, 512, kernel_size=(4,4), stride=(2,2), padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 256, kernel_size=(4,4), stride=(2,2), padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
