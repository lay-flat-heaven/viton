...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.stage == 'GMM':
            ...
            name = im_name[:-4] + "_" + c_name[:-4] + ".jpg"
            c = Image.open(osp.join(self.data_path, 'warp-cloth', name))
            cm = Image.open(osp.join(self.data_path, 'warp-mask', name))
...
class CPDataLoader(object):
    def __init__(self, opt, dataset):
        ...
        self.data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=opt.batch_size, shuffle=opt.shuffle,
